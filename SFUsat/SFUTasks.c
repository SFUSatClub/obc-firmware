...
void vSerialTask(void *pvParameters) {
    ...
    BaseType_t xStatus;
    while(1) {
        xStatus = xQueueReceive(xSerialQueue, &queuedStr, xTicksToWait);
        if(xStatus == pdPASS) {
            serialSendln(queuedStr);
            //free(queuedStr);
//            char nm[30];
...
//            serialSendln(nm);
            if(uxQueueMessagesWaiting(xQueue) > 5) {
                serialSendln("WARNING: lots of serial queue msgs");
            }
        }
    }
}
...
BaseType_t serialSendQ(char * toSend) {
    if(xQueueSendToBack(xSerialQueue, &toSend, 0) == pdPASS) {
        return pdPASS;
    } else {
        return pdFAIL;
    }
