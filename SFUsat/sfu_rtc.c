...
const uint32_t cumulativeSecondsInMonth[12] = {0, 2678400, 5097600, 7776000, 10368000, 13046400, 15638400, 18316800, 20995200, 23587200, 26265600, 28857600}; // zero, seconds in january, then seconds in jan + feb, then jan + feb + mar
// in feb, we add seconds in jan plus the current time in feb to get the seconds since start


// ---------------------- Original file from Steven -------------
...
uint32_t getEpoch(){
    ...
    uint32_t nonLeapYear = epochOffset + (year*31536000) + (cumulativeSecondsInMonth[(rtc_get_month()-1)]) + ((rtc_get_day() -1)*86400) + (rtc_get_hours()*3600) + (rtc_get_minutes()*60) + rtc_get_seconds();
